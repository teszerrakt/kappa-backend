...
from kappa.cluster import merge_cluster


def find_neighbor(
    item_id, ratings, usage="prediction", metric="correlation", k=10, verbose=False
):
    similarities = []
    indices = []
    ...
    distances, indices = model_knn.kneighbors(
        ratings.iloc[loc, :].values.reshape(1, -1), n_neighbors=n_neighbors
    )
    ...
    if verbose:
        ...
        for i in range(0, len(indices.flatten())):
            if indices.flatten()[i] + 1 == item_id:
                ...
                print(
                    "{}: {}, with similarity of {}".format(
                        i,
                        ratings.index[indices.flatten()[i]],
...
def predict(
    ...
    clustered_ratings = ratings_cluster.loc[ratings_cluster["cluster"] == which_cluster]
    ...
    for i in range(0, len(indices.flatten())):
        ...
        if (indices.flatten()[i] == item_loc) or (
            clustered_ratings.iloc[indices.flatten()[i], user_loc] == 0
        ):
            continue
        else:
            # rating * similarity
            product = (
                clustered_ratings.iloc[indices.flatten()[i], user_loc]
                * (similarities[i])
            )
            ...
            if verbose:
                print(
                    "{}. item_loc: {}, user_loc: {} -> rating: {} * similarity: {} = {}".format(
                        i,
                        indices.flatten()[i],
                        user_loc,
                        clustered_ratings.iloc[indices.flatten()[i], user_loc],
    ...
    if sum_wt == 0:
        prediction = (
            0  # if the sum of similarity == 0, then the predicted rating is also 0
        )
    else:
    ...
    if verbose:
        print(
            "Predicted rating for user {} -> item {}: {}".format(
